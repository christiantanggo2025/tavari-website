// src/App.jsx - EXACT WORKING VERSION
...
function App() {
  ...
  useEffect(() => {
    // Get both session AND user to ensure everything is loaded
    const initAuth = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      const { data: { user } } = await supabase.auth.getUser();
      
      // Use whichever has the user data
      const finalSession = session || (user ? { user } : null);
    ...
    <BusinessProvider>
      <Routes>
        ...
        >
          ...
          <Route path="mail/templates" element={<CampaignList />} /> {/* Will create separate component later */}
          <Route path="mail/compliance" element={<ContactsDashboard />} /> {/* Will create separate component later */}
          <Route path="mail/billing" element={<BillingManager />} />
          <Route path="mail/settings" element={<MailSettings />} />
          <Route path="/dashboard/mail/campaigns/:campaignId" element={<CampaignDetails />} />
          
          {/* NEW: Steps 132-133 Mail Routes */}
